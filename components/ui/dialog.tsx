...
import { useTabPortalContainer } from '@/components/tab-portal-context'
...
function DialogContent({
  ...
  overlayClassName,
  portalContainer,
  disableTabPortal = false,
  ...props
}: React.ComponentProps<typeof DialogPrimitive.Content> & {
  showCloseButton?: boolean
  overlayClassName?: string
  portalContainer?: HTMLElement | null
  disableTabPortal?: boolean
}) {
  const tabContainer = useTabPortalContainer()
  const container = disableTabPortal
    ? portalContainer ?? null
    : portalContainer ?? tabContainer ?? null
  const isScoped = Boolean(container)

  return (
    <DialogPortal data-slot="dialog-portal" container={container ?? undefined}>
      <DialogOverlay
        className={cn(
          isScoped ? 'absolute inset-0' : 'fixed inset-0',
          overlayClassName,
        )}
      />
